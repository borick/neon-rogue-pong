...
const MatrixBreachCanvas: React.FC<MatrixBreachProps> = ({ onVictory, onDamage }) => {
  ...
  const state = useRef({
    ...
    sentinels: [
      { x: 250, y: 150, r: 85, angle: 0, speed: 0.03 },
      { x: 450, y: 450, r: 110, angle: Math.PI, speed: -0.02 },
      { x: 650, y: 300, r: 130, angle: 0, speed: 0.015 },
    ],
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const nextX = p.x + dx; const nextY = p.y + dy;
      let canMoveX = true; let canMoveY = true;
      WALLS.forEach(w => {
        if (nextX + p.size > w.x && nextX - p.size < w.x + w.w && p.y + p.size > w.y && p.y - p.size < w.y + w.h) canMoveX = false;
        if (p.x + p.size > w.x && p.x - p.size < w.x + w.w && nextY + p.size > w.y && nextY - p.size < w.y + w.h) canMoveY = false;
      });
      ...
      s.packets.forEach(pkt => {
        if (!pkt.collected && Math.hypot(p.x - pkt.x, p.y - pkt.y) < 20) {
          pkt.collected = true; SoundSystem.playUiClick();
        }
      });

      s.sentinels.forEach(sen => {
        sen.angle += sen.speed;
        const sx = sen.x + Math.cos(sen.angle) * 60;
        const sy = sen.y + Math.sin(sen.angle) * 60;
        if (Math.hypot(p.x - sx, p.y - sy) < sen.r) {
          p.x = 50; p.y = CANVAS_HEIGHT / 2;
          onDamage(1); SoundSystem.playPlayerHit();
        }
      });

      if (s.packets.every(pkt => pkt.collected) && p.x > CANVAS_WIDTH - 50) { onVictory(); return; }

      ctx.fillStyle = '#020502'; ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
      ctx.strokeStyle = '#0a200a'; ctx.lineWidth = 1;
      ...
      ctx.fillStyle = '#1a3a1a'; ctx.strokeStyle = '#4ade80'; ctx.lineWidth = 2;
      WALLS.forEach(w => { ctx.fillRect(w.x, w.y, w.w, w.h); ctx.strokeRect(w.x, w.y, w.w, w.h); });

      s.sentinels.forEach(sen => {
        const sx = sen.x + Math.cos(sen.angle) * 60; const sy = sen.y + Math.sin(sen.angle) * 60;
        ctx.fillStyle = 'rgba(239, 68, 68, 0.2)'; ctx.beginPath(); ctx.arc(sx, sy, sen.r, 0, Math.PI * 2); ctx.fill();
        ctx.strokeStyle = '#ef4444'; ctx.stroke();
      });

      s.packets.forEach(pkt => {
        if (!pkt.collected) { ctx.fillStyle = '#facc15'; ctx.shadowBlur = 15; ctx.shadowColor = '#facc15'; ctx.fillRect(pkt.x - 5, pkt.y - 5, 10, 10); ctx.shadowBlur = 0; }
      });

      ctx.fillStyle = '#fff'; ctx.shadowBlur = 20; ctx.shadowColor = '#4ade80';
